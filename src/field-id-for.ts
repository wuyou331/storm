import { Expression, assertExpression, assertArrowFunctionExpression, ArrowFunctionExpressionNode, ExpressionKind } from "tst-expression";
...
export function fieldIdFor<TModel>(memberExpression: Expression<(m: TModel) => any>) {
	assertExpression(memberExpression);
	assertArrowFunctionExpression(memberExpression.expression);
